...
window.submitApplication = async function(toEmail) {
  ...
  /* --- reCAPTCHA v3 --- */
  const recaptchaToken = await getRecaptchaToken('apply').catch(() => null);
...
window.submitPartnerEnquiry = async function(toEmail) {
  ...
  /* --- reCAPTCHA v3 --- */
  const recaptchaToken = await getRecaptchaToken('partner_enquiry').catch(() => null);
...
window.submitQuoteRequest = async function(toEmail) {
  ...
  /* --- reCAPTCHA v3 --- */
  const recaptchaToken = await getRecaptchaToken('quote_request').catch(() => null);
