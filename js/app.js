...
function safeHistoryPush(state, hash) {
  try { history.pushState(state, '', hash); } catch(e) {}
}
function safeHistoryReplace(state, hash) {
  try { history.replaceState(state, '', hash); } catch(e) {}
}
...
function updateMeta(page, tab) {
  ...
  const url   = 'https://www.tenetnetworks.com/' + hash;
...
function nav(page, tab) {
  ...
  const hash  = tab ? `#${page}/${tab}` : `#${page}`;
  safeHistoryPush(state, hash);
  render(page, tab);
  updateMeta(page, tab);
  
  /* Scroll behavior:
     - If navigating to a specific tab: scroll to top and let user see the tab content
     - If no tab specified: restore saved scroll position or go to top
  */
  requestAnimationFrame(() => {
    setTimeout(() => {
      // If navigating to a specific tab, always go to top to show the tab content
      if (tab) {
        console.log(`[SCROLL] Navigating to ${page}/${tab} - scrolling to top`);
...
window.addEventListener('popstate', e => {
  const state = e.state || { page: 'home', tab: null };
  currentPage = state.page;
  render(state.page, state.tab);
  updateMeta(state.page, state.tab);
...
function switchTab(tabId) {
  ...
  const hash = `#${currentPage}/${tabId}`;
  safeHistoryReplace({ page: currentPage, tab: tabId }, hash);
...
function productPanelHTML(item, ctaHTML) {
  ...
  // Render capabilities grid if capabilities exist
    ...
    <div class="footer-grid">
      <div class="footer-brand">
        ...
        <div class="socials">
          ...
          <a href="${c.twitter}" target="_blank" rel="noopener noreferrer" class="soc" title="Twitter">𝕏</a>
      ...
      <div class="footer-col">
        ...
        <a data-nav="products" data-tab="cwan">cWAN / SD-WAN</a>
...
function renderHome() {
  ...
  const svcCards = ['custom','consulting','software'].map(id=>{
    ...
    return `<div class="card" data-nav="services" data-tab="${id}">
      ...
      <h3>${s.label}</h3><p>${s.subhead} ${s.body.split('.')[0]}.</p>
      <span class="card-link">Learn more</span>
    </div>`;
  }).join('');
      ...
      <div class="section-inner">
        <div class="about-grid">
          <div>
            ...
            <h2 style="margin:.6rem 0 1rem">Making Technology Work <span class="accent">Seamlessly</span> for You</h2>
            <p class="body-l">With over 150 years of combined technology leadership experience, our team brings deep expertise in embedded systems, network engineering, and scalable system design. From M2M gateways to Multi-WAN solutions, we deliver products and services that just work — securely, reliably, and at scale.</p>
            <p class="body-m" style="margin-top:.9rem">From conceptualizing to deploying and maintaining advanced systems, we're committed to ensuring the best in connectivity and automation. Our goal is simple yet powerful: to unlock value for our customers by delivering best-in-class solutions.</p>
            <ul class="about-cap-list">
              <li><div class="cap-icon">🏭</div>Industrial-grade M2M &amp; Edge Devices</li>
              <li><div class="cap-icon">🔄</div>Legacy-to-Digital Network Transformation</li>
              <li><div class="cap-icon">⚙️</div>Custom Engineering &amp; Firmware Development</li>
              <li><div class="cap-icon">📡</div>Consulting in LTE, 5G NR, and IoT Systems</li>
            </ul>
            <div class="btn-group" style="margin-top:2rem">
...
function renderServices(defaultTab='custom') {
  ...
  const panels = Object.keys(S).map(id =>
    tabPanel(id, productPanelHTML({...S[id], specs:[], features:S[id].features, badge:S[id].badge, label:S[id].label, subhead:S[id].subhead, body:S[id].body}, serviceCTA).replace('<h4 style="font-family:var(--font-h);font-size:.75rem;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);margin-bottom:.3rem">Specifications</h4><div class="spec-table"></div>',''),
    defaultTab===id)
  ).join('');
...
function renderCompany(defaultTab='about') {
  ...
  const careersHTML = `
    ...
    <h3 style="margin-bottom:1.2rem;color:var(--secondary)">Join Our Team</h3>
    <div class="grid-auto" style="margin-bottom:2rem">${programs}</div>
    <div class="cta-banner">
      <div>
        <h2>Ready to Start Your Journey?</h2>
        <p>Send your resume to <a href="mailto:${CONTENT.company.careers}" style="color:var(--secondary)">${CONTENT.company.careers}</a></p>
      </div>
...
/* ── INIT ── */
/* Parse the current URL hash to restore state on page load/refresh */
function initFromHash() {
  ...
  safeHistoryReplace({ page, tab }, hash);
  currentPage = page;
  render(page, tab);
  updateMeta(page, tab);
}
