window.CONTENT = {
  ...
  */
  ...
  stories: (function() {
    // This runs immediately and returns the stories array
    let loadedStories = [];
    
    // Fetch stories synchronously (works because content.js loads before app.js)
    const xhr = new XMLHttpRequest();
    // Support both http:// and file:// protocols
    const storiesPath = window.location.protocol === 'file:' 
      ? 'stories/stories.json'  // relative path for file://
      : '/stories/stories.json'; // absolute for http://
    
    xhr.open('GET', storiesPath, false); // false = synchronous
    xhr.send(null);
    
    if (xhr.status === 200 || xhr.status === 0) { // 0 = success for file://
      loadedStories = JSON.parse(xhr.responseText);
      console.log('✅ Loaded', loadedStories.length, 'customer stories from /stories/stories.json');
    } else {
