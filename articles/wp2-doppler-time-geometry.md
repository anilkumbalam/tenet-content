...
For a carrier frequency $f_0$, the Doppler shift $\Delta f$ is approximately:

$$\Delta f = \frac{v_r}{c} f_0$$

where:

- $v_r$ = relative radial velocity $^{C}$
...
$$v_{\text{radial}} = \mathbf{v}_{\text{rel}} \cdot \hat{\mathbf{r}}$$
...
$$\Delta f \approx \frac{v_{\text{radial}}}{c} f_0$$
where:
